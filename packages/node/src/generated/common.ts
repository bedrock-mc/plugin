...
export const CustomBlockProperties: MessageFns<CustomBlockProperties> = {
  encode(message: CustomBlockProperties, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    globalThis.Object.entries(message.states).forEach(([key, value]: [string, CustomBlockStateValues]) => {
  ...
  fromJSON(object: any): CustomBlockProperties {
    return {
      ...
      states: isObject(object.states)
        ? (globalThis.Object.entries(object.states) as [string, any][]).reduce(
          (acc: { [key: string]: CustomBlockStateValues }, [key, value]: [string, any]) => {
  ...
  toJSON(message: CustomBlockProperties): unknown {
    ...
    if (message.states) {
      const entries = globalThis.Object.entries(message.states) as [string, CustomBlockStateValues][];
      if (entries.length > 0) {
        obj.states = {};
        entries.forEach(([k, v]) => {
  ...
  fromPartial(object: DeepPartial<CustomBlockProperties>): CustomBlockProperties {
    ...
    message.states = (globalThis.Object.entries(object.states ?? {}) as [string, CustomBlockStateValues][]).reduce(
      (acc: { [key: string]: CustomBlockStateValues }, [key, value]: [string, CustomBlockStateValues]) => {
        if (value !== undefined) {
